...
static int save_frame_as_jpeg(AVCodecContext *pCodecCtx, AVFrame *pFrame);

int main(int argc, const char *argv[])
{
    if (argc < 2)
    {
        printf("You need to specify a media file or stream address\n");
        return -1;
    }
...
static int decode_packet(AVPacket *pPacket, AVCodecContext *pCodecContext, AVFrame *pFrame)
{
    ...
    while (response >= 0)
    {
        ...
        if (response >= 0)
        {
            ...
            {
                logging(
                    "Warning: the generated file may not be a grayscale image, but could e.g. be "
                    "just the R component if the video format is RGB");
            }
            // save a grayscale frame into a .pgm file
            save_gray_frame(pFrame->data[0], pFrame->linesize[0], pFrame->width, pFrame->height,
                            frame_filename);

            save_frame_as_jpeg(pCodecContext, pFrame);
        }
    }
    return 0;
}
...
// https://stackoverflow.com/questions/74391719/saving-frames-as-jpg-with-ffmpeg-visual-studio-c
...
static int save_frame_as_jpeg(AVCodecContext *pCodecCtx, AVFrame *pFrame)
{
    ...
    if (!jpegCodec)
    {
        logging("ERROR could not find encoder AV_CODEC_ID_MJPEG: %d", ret);
        return -1;
    }
    ...
    if (!jpegContext)
    {
        logging("ERROR could not alloc jpegCodec context: %d", ret);
        return -1;
    }
    ...
    if (ret < 0)
    {
        logging("ERROR could not open jpeg context: %d", ret);
        return ret;
    }
    FILE *JPEGFile;
    char JPEGFName[256];
    ...
    if (ret < 0)
    {
        logging("ERROR could not send frame to jpegContext: %d", ret);
        return ret;
    }
    ...
    sprintf(JPEGFName, "c:\\folder\\dvr-%06d.jpg", pCodecCtx->frame_number);
